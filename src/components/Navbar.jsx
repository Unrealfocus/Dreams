      ...
      <ul className="list-none sm:flex hidden justify-end items-center flex-1">
        {navLinks.map((nav, index) => (
          <li
            ...
            onClick={() => setActive((prev)=>prev=nav.title)}
          >
            <a href={`#${nav.id}`} className="text-white no-underline cursor-pointer">{nav.title}</a>
          </li>
        ))}
      </ul>
        ...
        >
          <ul className="list-none flex justify-end items-start flex-1 flex-col">
            {navLinks.map((nav, index) => (
              ...
              >
                <a href={`#${nav.id}`} className="text-white no-underline">{nav.title}</a>
              </li>
            ))}
          </ul>
        </div>
